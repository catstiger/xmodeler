...
"IntermediateCatchEvent")return"TimerEvent";return a},convert:function(a,b,c,e){if(a&&a.flowElements&&a.flowElements.length!=0){var j;j=e?e:c.stage;for(var i=[],g=0;g<a.flowElements.length;g++){var d=a.flowElements[g],f=b.locationMap[d.id];if(f){var h=this._extractBpmnType(b,d),h=this._fixType(h,d),m=f.x;e&&(m=f.x-e.x);var k=f.y;e&&(k=f.y-e.y);f=c._addAcitivty(h,{id:d.id,name:d.name,x:m,y:k,width:f.width,height:f.height},j);f.raw={};f.raw.documentation=d.documentation;f.raw.defaultFlow=d.defaultFlow;
...
f.raw.usertaskassignment.items.length,f.raw.formkeydefinition=d.formKey,f.raw.exclusivedefinition=!d.notExclusive,f.raw.asynchronousdefinition=d.asynchronous,f.raw.duedatedefinition=d.dueDate,f.raw.prioritydefinition=d.priority,d.taskListeners&&d.taskListeners.length>0){f.raw.tasklisteners={totalCount:d.taskListeners.length,items:[]};for(h=0;h<d.taskListeners.length;h++){var k=d.taskListeners[h],n={task_listener_event_type:k.event};if(k.implementationType==="delegateExpression")n.task_listener_delegate_expression=
k.implementation;else if(k.implementationType==="expression")n.task_listener_expression=k.implementation;else if(k.implementationType==="class")n.task_listener_class=k.implementation;f.raw.tasklisteners.items.push(n)}}}else if(h==="ServiceTask")if(d.implementationType==="class")f.raw.servicetaskclass=d.implementation;else if(d.implementationType==="expression")f.raw.servicetaskexpression=d.implementation;else{if(d.implementationType==="delegateExpression")f.raw.servicetaskdelegateexpression=d.implementation}else if(h===
"ScriptTask")f.raw.scripttext=d.script,f.raw.scriptformat=d.scriptFormat;else if(h==="TimerEvent")i.push({node:f,element:d});else if(h==="StartEvent")f.raw.formkeydefinition=d.formKey,f.raw.initiator=d.initiator;if(typeof d.formProperties!="undefined"&&d.formProperties.length>0)for(m in f.raw.formproperties={totalCount:d.formProperties.length,items:[]},d.formProperties)h=d.formProperties[m],f.raw.formproperties.items.push({formproperty_id:parseInt(h.id)?parseInt(h.id):h.id,formproperty_name:h.name,
formproperty_type:"",formproperty_expression:"",formproperty_variable:""});this.doExecutionListeners(d,f);d.flowElements&&d.flowElements.length>0&&this.convert(d,b,c,f)}}for(g=0;g<i.length;g++)i[g].node.raw.cancelactivity=i[g].element.cancelActivity,i[g].node.raw.timerdurationdefinition=i[g].element.eventDefinitions[0].timeDuration,i[g].node.raw.timerdatedefinition=i[g].element.eventDefinitions[0].timeDate,i[g].node.raw.timercycledefinition=i[g].element.eventDefinitions[0].timeCycle,i[g].element.attachedToRef&&
i[g].element.attachedToRefId&&(d=c.byId(i[g].element.attachedToRefId))&&i[g].node._dock(d);for(g=0;g<a.flowElements.length;g++)if(d=a.flowElements[g],f=b.flowLocationMap[d.id]){e=i=null;m=[];d.sourceRef&&(i=c.byId(d.sourceRef));d.targetRef&&(e=c.byId(d.targetRef));for(h=0;h<f.length;h++)m.push({x:f[h].x,y:f[h].y});if(i&&e){var l=c.addSequenceFlow({startNode:i,endNode:e,points:m,id:d.id},j);l.condition=d.conditionExpression;l.raw.conditionsequenceflow=d.conditionExpression;l.raw.documentation=d.documentation;
