...
d.implementation}else if(i==="ScriptTask")e.raw.scripttext=d.script,e.raw.scriptformat=d.scriptFormat;else if(i==="TimerEvent")h.push({node:e,element:d});else if(i==="StartEvent")e.raw.formkeydefinition=d.formKey,e.raw.initiator=d.initiator;if(typeof d.formProperties!="undefined"&&d.formProperties.length>0)for(k in e.raw.formproperties={totalCount:d.formProperties.length,items:[]},d.formProperties)i=d.formProperties[k],e.raw.formproperties.items.push({formproperty_id:parseInt(i.id)?parseInt(i.id):
i.id,formproperty_name:i.name,formproperty_type:"",formproperty_expression:"",formproperty_variable:""});d.flowElements&&d.flowElements.length>0&&this.convert(d,b,c,e)}}for(g=0;g<h.length;g++)h[g].node.raw.cancelactivity=h[g].element.cancelActivity,h[g].node.raw.timerdurationdefinition=h[g].element.eventDefinitions[0].timeDuration,h[g].node.raw.timerdatedefinition=h[g].element.eventDefinitions[0].timeDate,h[g].node.raw.timercycledefinition=h[g].element.eventDefinitions[0].timeCycle,h[g].element.attachedToRef&&
h[g].element.attachedToRefId&&(d=c.byId(h[g].element.attachedToRefId))&&h[g].node._dock(d);for(g=0;g<a.flowElements.length;g++)if(d=a.flowElements[g],e=b.flowLocationMap[d.id]){f=h=null;k=[];d.sourceRef&&(h=c.byId(d.sourceRef));d.targetRef&&(f=c.byId(d.targetRef));for(i=0;i<e.length;i++)k.push({x:e[i].x,y:e[i].y});if(h&&f)e=c.addSequenceFlow({startNode:h,endNode:f,points:k,id:d.id},j),e.condition=d.conditionExpression,e.raw.conditionsequenceflow=d.conditionExpression,e.raw.documentation=d.documentation,
e.raw.name=d.name,e.name=d.name,e.isDefault=e.id===e.startNode.raw.defaultFlow}}},convertRoot:function(a,b){a.raw.process_id=b.mainProcess.id;a.raw.process_namespace=b.targetNamespace||"http://www.activiti.org/processdef";a.raw.name=b.mainProcess.name||"";a.raw.documentation=b.mainProcess.documentation||"";a.raw.candidateStarterUsers=b.mainProcess.candidateStarterUsers||"";a.raw.candidateStarterGroups=b.mainProcess.candidateStarterGroups||"";var c=a.raw.candidateStarterUsers;if(Ext.isArray(c)&&c.length>
0)a.raw.candidateStarterUsers=c.join(",");c=a.raw.candidateStarterGroups;!Ext.isArray(c)&&typeof c==="string"&&(c=c.split(","));if(c&&c.length>0){for(var f=[],j=0;j<c.length;j++)isNaN(parseInt(c[j]))||f.push(parseInt(c[j]));a.raw.candidateStarterGroups=f}},load:function(a,b){var c={};if(typeof b.processDefinitionId!="undefined"&&b.processDefinitionId!=null)c.processDefinitionId=b.processDefinitionId;else if(typeof b.deploymentId!="undefined"&&b.deploymentId!=null)c.deploymentId=b.deploymentId;else if(typeof b.key!=
"undefined"&&b.key!=null)c.key=b.key;else if(typeof b.processInstanceId!="undefined"&&b.processInstanceId!=null)c.processInstanceId=b.processInstanceId;else{alert("\u6ca1\u6709\u8db3\u591f\u7684\u53c2\u6570\uff01");return}var f=this;Ext.Ajax.request({url:b.queryUrl,params:c,method:"GET",success:function(c){c=Ext.decode(c.responseText,!0);!c||c.msg?Utils.isFunction(b.loadFailed)&&b.loadFailed(c):(a.clear(),new ActivityAdapter,f.convert(c.mainProcess,c,a),Utils.isFunction(b.loadSuccess)&&(f.convertRoot(a,
c),b.loadSuccess(c)))},failure:function(a){Utils.isFunction(b.loadFailed)&&b.loadFailed(a)}})}};
