...
"IntermediateCatchEvent")return"TimerEvent";return a},convert:function(a,b,c,e){if(a&&a.flowElements&&a.flowElements.length!=0){var j;j=e?e:c.stage;for(var i=[],h=0;h<a.flowElements.length;h++){var d=a.flowElements[h],f=b.locationMap[d.id];if(f){var g=this._extractBpmnType(b,d),g=this._fixType(g,d),m=f.x;e&&(m=f.x-e.x);var k=f.y;e&&(k=f.y-e.y);f=c._addAcitivty(g,{id:d.id,name:d.name,x:m,y:k,width:f.width,height:f.height},j);f.raw={};f.raw.documentation=d.documentation;f.raw.defaultFlow=d.defaultFlow;
f.raw.initiator=d.initiator;if(g==="UserTask"){if(f.raw.usertaskassignment={items:[]},d.assignee&&f.raw.usertaskassignment.items.push({assignment_type:"assignee",resourceassignmentexpr:d.assignee}),d.candidateUsers&&f.raw.usertaskassignment.items.push({assignment_type:"candidateUsers",resourceassignmentexpr:d.candidateUsers.join(",")}),d.candidateGroups&&f.raw.usertaskassignment.items.push({assignment_type:"candidateGroups",resourceassignmentexpr:d.candidateGroups.join(",")}),f.raw.usertaskassignment.totalCount=
f.raw.usertaskassignment.items.length,f.raw.formkeydefinition=d.formKey,f.raw.exclusivedefinition=!d.notExclusive,f.raw.asynchronousdefinition=d.asynchronous,f.raw.duedatedefinition=d.dueDate,f.raw.prioritydefinition=d.priority,d.taskListeners&&d.taskListeners.length>0){f.raw.tasklisteners={totalCount:d.taskListeners.length,items:[]};for(g=0;g<d.taskListeners.length;g++){var k=d.taskListeners[g],n={task_listener_event_type:k.event};if(k.implementationType==="delegateExpression")n.task_listener_delegate_expression=
k.implementation;else if(k.implementationType==="expression")n.task_listener_expression=k.implementation;else if(k.implementationType==="class")n.task_listener_class=k.implementation;f.raw.tasklisteners.items.push(n)}}}else if(g==="ServiceTask")if(d.implementationType==="class")f.raw.servicetaskclass=d.implementation;else if(d.implementationType==="expression")f.raw.servicetaskexpression=d.implementation;else{if(d.implementationType==="delegateExpression")f.raw.servicetaskdelegateexpression=d.implementation}else g===
"ScriptTask"?(f.raw.scripttext=d.script,f.raw.scriptformat=d.scriptFormat):g==="TimerEvent"?i.push({node:f,element:d}):g==="StartEvent"?(f.raw.formkeydefinition=d.formKey,f.raw.initiator=d.initiator):g==="EndEvent"&&console.log(d);if(typeof d.formProperties!="undefined"&&d.formProperties.length>0)for(m in f.raw.formproperties={totalCount:d.formProperties.length,items:[]},d.formProperties)g=d.formProperties[m],f.raw.formproperties.items.push({formproperty_id:parseInt(g.id)?parseInt(g.id):g.id,formproperty_name:g.name,
formproperty_type:"",formproperty_expression:"",formproperty_variable:""});this.doExecutionListeners(d,f);d.flowElements&&d.flowElements.length>0&&this.convert(d,b,c,f)}}for(h=0;h<i.length;h++)i[h].node.raw.cancelactivity=i[h].element.cancelActivity,i[h].node.raw.timerdurationdefinition=i[h].element.eventDefinitions[0].timeDuration,i[h].node.raw.timerdatedefinition=i[h].element.eventDefinitions[0].timeDate,i[h].node.raw.timercycledefinition=i[h].element.eventDefinitions[0].timeCycle,i[h].element.attachedToRef&&
i[h].element.attachedToRefId&&(d=c.byId(i[h].element.attachedToRefId))&&i[h].node._dock(d);for(h=0;h<a.flowElements.length;h++)if(d=a.flowElements[h],f=b.flowLocationMap[d.id]){e=i=null;m=[];d.sourceRef&&(i=c.byId(d.sourceRef));d.targetRef&&(e=c.byId(d.targetRef));for(g=0;g<f.length;g++)m.push({x:f[g].x,y:f[g].y});if(i&&e){var l=c.addSequenceFlow({startNode:i,endNode:e,points:m,id:d.id},j);l.condition=d.conditionExpression;l.raw.conditionsequenceflow=d.conditionExpression;l.raw.documentation=d.documentation;
l.raw.name=d.name;l.name=d.name;l.isDefault=l.id===l.startNode.raw.defaultFlow}this.doExecutionListeners(d,l)}}},doExecutionListeners:function(a,b){if(a.executionListeners&&a.executionListeners.length>0){b.raw.executionlisteners={totalCount:a.executionListeners.length,items:[]};for(var c=0;c<a.executionListeners.length;c++){var e=a.executionListeners[c],j={execution_listener_event_type:e.event};if(e.implementationType==="delegateExpression")j.execution_listener_delegate_expression=e.implementation;
else if(e.implementationType==="expression")j.execution_listener_expression=e.implementation;else if(e.implementationType==="class")j.execution_listener_class=e.implementation;b.raw.executionlisteners.items.push(j)}}},convertRoot:function(a,b){a.raw.process_id=b.mainProcess.id;a.raw.process_namespace=b.targetNamespace||"http://www.activiti.org/processdef";a.raw.name=b.mainProcess.name||"";a.raw.documentation=b.mainProcess.documentation||"";a.raw.candidateStarterUsers=b.mainProcess.candidateStarterUsers||
"";a.raw.candidateStarterGroups=b.mainProcess.candidateStarterGroups||"";var c=a.raw.candidateStarterUsers;if(Ext.isArray(c)&&c.length>0)a.raw.candidateStarterUsers=c.join(",");c=a.raw.candidateStarterGroups;!Ext.isArray(c)&&typeof c==="string"&&(c=c.split(","));if(c&&c.length>0){for(var e=[],j=0;j<c.length;j++)isNaN(parseInt(c[j]))||e.push(parseInt(c[j]));a.raw.candidateStarterGroups=e}},load:function(a,b){var c={};if(typeof b.processDefinitionId!="undefined"&&b.processDefinitionId!=null)c.processDefinitionId=
b.processDefinitionId;else if(typeof b.deploymentId!="undefined"&&b.deploymentId!=null)c.deploymentId=b.deploymentId;else if(typeof b.key!="undefined"&&b.key!=null)c.key=b.key;else if(typeof b.processInstanceId!="undefined"&&b.processInstanceId!=null)c.processInstanceId=b.processInstanceId;else{alert("\u6ca1\u6709\u8db3\u591f\u7684\u53c2\u6570\uff01");return}var e=this;Ext.Ajax.request({url:b.queryUrl,params:c,method:"GET",success:function(c){c=Ext.decode(c.responseText,!0);!c||c.msg?Utils.isFunction(b.loadFailed)&&
b.loadFailed(c):(a.clear(),new ActivityAdapter,e.convert(c.mainProcess,c,a),Utils.isFunction(b.loadSuccess)&&(e.convertRoot(a,c),b.loadSuccess(c)))},failure:function(a){Utils.isFunction(b.loadFailed)&&b.loadFailed(a)}})}};
