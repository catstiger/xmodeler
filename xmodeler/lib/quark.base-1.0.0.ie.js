...
{
    if(Utils.isIE) {
        Utils.stopEvent(e, continueDefault, continuePropagation);
    } else {
...
Timer.prototype.start = function()
{
	...
	this._startTime = this.info.lastTime = this.info.currentTime = Utils.now();
	var me = this;
...
Timer.prototype._run = function()
{
	...
	var info = this.info;
	var time = info.currentTime = Utils.now();
...
{
	var obj = {step:callback, __runTime:Utils.now() + time};
...
{
	this._startTime = Utils.now() + this.delay;
	this._pausedTime = 0;
...
{	
	this.paused = true;
	this._pausedStartTime = Utils.now();
};
...
{	
	this.paused = false;
	this._pausedTime += Utils.now() - this._pausedStartTime;
};
...
function isDrawable(elem)
{
	...
	if(Utils.isIE8m) {
        return (elem.tagName == 'IMG' || elem.tagName == 'CANVAS' || elem.tagName == 'VIDEO');
    } else {
        return (elem instanceof HTMLImageElement) || (elem instanceof HTMLCanvasElement) || (elem instanceof HTMLVideoElement);
    }
...
Graphics.prototype.cache = function(toImage)
{
	...
	if(Utils.isIE8m) {
	    this._draw(Utils.getContext(canvas));
	} else {
...
{
    var dom = Quark.createDOM("canvas");
    var ctx;
    if(Utils.isIE8m) {
        ctx = Utils.getContext(dom);
    } else {
