...
GraphicsEx.prototype.dashedLine=function(a,b,c,d,e){e||(e=[5,2]);var f=e.length;this._addAction(["beginPath"]);this._addAction(["moveTo",a,b]);for(var g=c-a,h=d-b,i=h/g,h=Math.sqrt(g*g+h*h),k=0,m=!0;h>=0.1;){var j=e[k++%f];j>h&&(j=h);if(g==0){var l=d>b?1:-1;b+=j*l}else{var n=Math.sqrt(j*j/(1+i*i)),l=c>a?1:-1;a+=n*l;b+=i*n*l}m?this._addAction(["lineTo",a,b]):this._addAction(["moveTo",a,b]);h-=j;m=!m}this._addAction(["closePath"]);return this};
GraphicsEx.prototype.dashedRect=function(a,b,c,d,e){e||(e=[5,2]);this._addAction(["beginPath"]);this.dashedLine(a,b,a+c,b,e).endFill();this.dashedLine(a,b,a,b+d,e).endFill();this.dashedLine(a+c,b,a+c,b+d,e).endFill();this.dashedLine(a,b+d,a+c,b+d,e).endFill();this._addAction(["closePath"]);return this};var Element=function(a){Element.superClass.constructor.call(this,a);this.basicPainter=new GraphicsEx({width:this.width,height:this.height,x:0,y:0});this.basicPainter.visible=!1;this.selectorPainter=new GraphicsEx({width:this.width,height:this.height,x:0,y:0});this.selectorPainter.visible=!1;this.addChild(this.basicPainter);this.addChild(this.selectorPainter);this.selected=!1;this.rawType=this.baseType=null;this.id=this.name=this.state="";this.startIndex=0;this.raw=a.raw||{}};Q.inherit(Element,Q.DisplayObjectContainer);
Element.prototype.onMouseDown=function(){};Element.prototype.onMouseUp=function(){};Element.prototype.onMouseEnter=function(){};Element.prototype.onMouseLeave=function(){};Element.prototype.onMouseMove=function(){};Element.prototype.onDblClick=function(){};Element.prototype.onKeyDown=function(){};Element.prototype.select=function(){this.selected=!0;Utils.isFunction(this._drawSelector)&&this._drawSelector()};
...
"none";var c=this.x,d=this.y,a=this.width,g=this.height;switch(this.resizeDirect){case "ne":d-=e;g+=e;a-=b;break;case "se":a-=b;g-=e;break;case "nw":d-=e;g+=e;c-=b;a+=b;break;case "sw":g-=e;c-=b;a+=b;break;case "s":g-=e;break;case "e":a-=b;break;case "n":d-=e;g+=e;break;case "w":c-=b,a+=b}if(Utils.isIE8m&&a>100&&g>60||!Utils.isIE8m&&a>60&&g>30)this.x=c,this.y=d,this.width=a,this.height=g;this.nameInputShowing&&(c=$("#node_name_input"),this._inputXY(c))}else if(Utils.isIE8m&&this.textDom&&this.visible===
...
AbstractActivity.prototype._drawText=function(){var a=function(a){a.preventDefault();a.stopPropagation();a.stopImmediatePropagation()};if(Utils.isIE6){var b="text-"+this.id;$(this.getStage().context.canvas).parent();if(!this.textDom)$(this.getStage().context.canvas).parent().append(['<span id="',b,'" class="canvas-text"></span>'].join("")),this.textDom=document.getElementById(b);if(!this.visible||this.resizing)this.textDom.style.display="none";else{var c=this.parent instanceof Quark.Stage?this.x:
this.parent.x+this.x,d=this.parent instanceof Quark.Stage?this.y:this.parent.y+this.y,b=0;!this.textAlign||this.textAlign=="middle"?b=d+this.height/2-6:this.textAlign=="top"&&(b=d+3);var e=this,d=$(this.textDom).innerWidth();$(this.textDom).css({"margin-left":c+this.width/2-d/2,"margin-top":b}).html(this.name).show().bind("mousedown",function(b){a(b);var c=e.x+b.offsetX,b=e.y+b.offsetY;e.getStage().context.canvas.engine.onMouseDown({offsetX:c,offsetY:b})}).bind("mouseup",function(b){a(b);var c=e.x+
b.offsetX,b=e.y+b.offsetY;e.getStage().context.canvas.engine.onMouseUp({offsetX:c,offsetY:b})}).bind("mousemove",function(b){a(b)}).bind("dblclick",function(b){a(b);e.onDblClick({offsetX:e.x+b.offsetX,offsetY:e.y+b.offsetY})}).bind("keydown",function(b){a(b);if(b.keyCode!=Q.KEY.DELETE||confirm("\u786e\u5b9a\u5220\u9664\u9009\u4e2d\u7684\u5bf9\u8c61\u5417\uff1f"))e.onKeyDown(b)}).bind("contextmenu",function(b){a(b);var c=e.x+b.offsetX,b=e.y+b.offsetY;e.getStage().context.canvas.engine.menu.showAt({offsetX:c,
...
a.endNode;this._destinations();this.arrowH=12;this.arrowW=8;this.painter=new Q.Graphics;this.addChild(this.painter);this.condition=null;this.draging=!1;this.dragingPoint=null;this.selected=!1;this.baseType="connection"};Q.inherit(AbstractConnection,Element);AbstractConnection.prototype.update=function(){this.points&&!(this.points.length<=0)&&(this._destinations(),this.painter.clear(),this.selected?(this._calcMiddle(),this._drawSelector()):this._drawBasic(this._isDefault()?"#D9442F":"#111111"),this._drawText())};
AbstractConnection.prototype.onMouseDown=function(a){if(Utils.isLeft(a)){var b=Utils.epos(a),b=this.hitTest(b.x,b.y);if(this.selected&&b)this.draging=b;this.selected=b;a.cancelBubble=b}};
...
AbstractConnection.prototype._calcHit=function(a,b){var c=[];if(!a||!b)return trace("\u53c2\u8003\u7ebf\u6bb5\u4e0d\u5b58\u5728\u6216\u8005\u53c2\u8003\u8282\u70b9\u4e0d\u5b58\u5728\uff01"),null;var d=a.x,e=a.y,f=b.x+b.width/2,g=b.y+b.height/2;c.push(this._calcCross(d,e,f,g,b.x,b.y,b.x+b.width,b.y));c.push(this._calcCross(d,e,f,g,b.x,b.y,b.x,b.y+b.height));c.push(this._calcCross(d,e,f,g,b.x+b.width,b.y,b.x+b.width,b.y+b.height));c.push(this._calcCross(d,e,f,g,b.x,b.y+b.height,b.x+b.width,b.y+b.height));
d=[];for(e=0;e<c.length;e++)(f=c[e])&&f.x>=b.x-1&&f.x<=b.x+b.width+1&&f.y>=b.y-1&&f.y<=b.y+b.height+1&&d.push({p:f,d:Math.sqrt((f.x-a.x)*(f.x-a.x)+(f.y-a.y)*(f.y-a.y))});return d.length==0?null:(c=Utils.min(d,"d"))?c.p:null};AbstractConnection.prototype._calcCross=function(a,b,c,d,e,f,g,h){if(a-c==0&&e-g==0)return null;if(b-d==0&&f-h==0)return null;b=(d*a-b*c)/(a-c);d=(d-b)/c;f=(h*e-f*g)/(e-g);h=(h-f)/g;a==c?c=h*a+f:(a=e==g?e:(f-b)/(d-h),c=d*a+b);return{x:a,y:c}};
...
AbstractConnection.prototype._drawText=function(){if(this.condition&&!Utils.isIE6){for(var a=0,b=0,c=0;c<this.points.length-1;c++){if(!this.points[c+1])return;var d=this.points[c],e=this.points[c+1],d=Math.sqrt((d.x-e.x)*(d.x-e.x)+(d.y-e.y)*(d.y-e.y));d>b&&(b=d,a=c)}a==0?(c=this.startNode.x+this.startNode.width/2,d=this.startNode.y+this.startNode.height/2):(c=this.points[a].x,d=this.points[a].y);var f=c,g=this.points[a+1].x,e=Math.atan((this.points[a+1].y-d)/(g-f)),h=this.getStage().context.canvas.getContext("2d").measureText(this.condition).width;
Utils.isIE8m&&(h*=1.5);a=g>f?a==0?this.startNode.width/2+10:b/2-h/2:a==0?-this.startNode.width/2-b/2-h/2-10:b/2-h/2;this.painter._addAction(["font","12px serif"])._addAction(["translate",c,d])._addAction(["rotate",e])._addAction(["fillText",this.condition,a,-10])}};
AbstractConnection.prototype._drawBasic=function(a){a||(a="#111111");this.painter.lineStyle(1,a);this.painter.beginPath()._addAction(["moveTo",this.points[0].x,this.points[0].y]);for(var b=1;b<this.points.length-1;b++){var c=this.points[b-1].x,d=this.points[b-1].y,e=this.points[b].x,f=this.points[b].y,f=(f-d)/(e-c),g=Math.atan(f),f=Math.cos(g)*10,g=Math.sin(g)*10,h=1;c>e&&(h=-1);var i=this.points[b].x-h*f,k=this.points[b].y-h*g;this.painter._addAction(["lineTo",i,k]);c=this.points[b].x;d=this.points[b].y;
e=this.points[b+1].x;f=this.points[b+1].y;f=(f-d)/(e-c);g=Math.atan(f);f=Math.cos(g)*10;g=Math.sin(g)*10;h=1;c>e&&(h=-1);this.painter._addAction(["bezierCurveTo",i,k,c,d,this.points[b].x+h*f,this.points[b].y+h*g])}this.painter._addAction(["lineTo",this.points[b].x,this.points[b].y]);this.painter.endFill();this._drawArrow(a)};
AbstractConnection.prototype._drawArrow=function(a){var b=this.points[this.points.length-2].x,c=this.points[this.points.length-1].x,d=this.points[this.points.length-1].y,e=(d-this.points[this.points.length-2].y)/(c-b),f=Math.atan(e)+Math.PI/9,e=Math.atan(e)-Math.PI/9,g=-1;b>c&&(g=1);var b=c+g*Math.cos(f)*10,f=d+g*Math.sin(f)*10,h=c+g*Math.cos(e)*10,e=d+g*Math.sin(e)*10;this.selected?this.painter.beginFill("#ff9900"):this.painter.beginFill(a);this.painter.beginPath()._addAction(["moveTo",c,d])._addAction(["lineTo",
b,f])._addAction(["lineTo",h,e]).closePath().endFill();this.painter.hasFill=!1};AbstractConnection.prototype._addCorner=function(a,b,c){for(var d=[],e=null,f=0;f<this.points.length;f++)d.push(this.points[f]),f==a&&(d.push({x:b,y:c}),e=f+1);this.points=d;return e};AbstractConnection.prototype._delCorner=function(a){for(var b=[],c=0;c<this.points.length;c++)c!=a&&b.push(this.points[c]);this.points=b};
...
AbstractConnection.prototype.inBox=function(a,b,c,d){if(!this.points||this.points.length==0)return!1;var e=this.points[0].x,f=this.points[0].y,g=e>=a&&f>=b&&e<=a+c&&f<=b+d,f=this.points.length,e=this.points[f-1].x,f=this.points[f-1].y;return g&&e>=a&&f>=b&&e<=a+c&&f<=b+d};
AbstractConnection.prototype.hitTest=function(a,b){for(var c=0;c<this.points.length-1;c++){var d,e=this.points[c].x>this.points[c+1].x?this.points[c+1].x:this.points[c].x,f=this.points[c].x>this.points[c+1].x?this.points[c].x:this.points[c+1].x,g=this.points[c].y>this.points[c+1].y?this.points[c+1].y:this.points[c].y,h=this.points[c].y>this.points[c+1].y?this.points[c].y:this.points[c+1].y,i=(this.points[c].x-this.points[c+1].x)/(this.points[c].y-this.points[c+1].y);Utils.equals(e,f)?d=a-e:Utils.equals(g,
h)?d=b-g:(d=Math.abs(Math.abs(i*(b-this.points[c].y)+this.points[c].x)-a),i=Math.abs(Math.abs(1/i*(a-this.points[c].x)+this.points[c].y)-b),d=d*i/Math.sqrt(d*d+i*i));if((isNaN(d)||Math.abs(d)<10)&&a>=e-2&&a<=f+2&&b>=g-2&&b<=h+2)return!0}return!1};AbstractConnection.prototype.getBounds=function(){return{lowerRight:{x:Utils.max(this.points,"x").x,y:Utils.max(this.points,"y").y},upperLeft:{x:Utils.min(this.points,"x").x,y:Utils.min(this.points,"y").y}}};
...
Q.inherit(ShadowFlow,Q.DisplayObjectContainer);ShadowFlow.create=function(a,b){var c={id:Quark.UIDUtil.createUID("shadowFlow"),x:0,y:0,autoSize:!0};a&&Q.merge(c,a);c=new ShadowFlow(c);c.visible=!0;if(b)c.startNode=b,c.begin=!0,c.startX=b.x+b.width/2,c.startY=b.y+b.height/2;return c};
...
a.addClass("curr")}).bind("mouseleave",function(){var a=$(this);a.hasClass("sp")||a.removeClass("curr")});$("ul.ctx-menu li").click(function(){var a=$(this);if(a.hasClass("delete")){var b=c.engine.getSelected();if(b&&b.length!=0){var d=b.length;if(confirm("\u662f\u5426\u786e\u5b9a\u5220\u9664\u8fd9\u4e9b\u5bf9\u8c61\uff1f"))for(a=0;a<d;a++){var e=b[a];e&&Utils.isFunction(e.remove)&&e.remove()}}}else if(a.hasClass("connection"))if((b=c.engine.getSelected())&&b.length>0)c.engine.addShadowFlow({removeOnComplete:!0},
b[0],b[0].parent);else{d=c.engine.stage.children.slice(0);for(a=0;a<d.length;a++)e=d[a],Element._isActivity(e)&&e.baseType==="container"&&(b=e.getSelected())&&b.length>0&&c.engine.addShadowFlow({removeOnComplete:!0},b[0],e)}else a.attr("action")&&c.addNode("add"+a.attr("action"))})}this.eventPos={};this.isShow=!1};
ContextMenu.prototype={menuDom:null,engine:null,addNode:function(a){var b=this.engine.getSelectedNodes();if(!b||b.length==0){var c={x:$(this.menuDom).offset().left-$(this.engine.container).offset().left,y:$(this.menuDom).offset().top-$(this.engine.container).offset().top};this.engine[a](c)}else{b=b[0];c={x:b.x+b.width+80,y:b.y+b.height/2-30};c=this.engine[a](c,b.parent);c.y=b.y+b.height/2-c.height/2;for(var d=!1,e=0;e<b.parent.children.length;e++){var f=b.parent.children[e];f.id!=b.id&&f.id!=c.id&&
Utils.isFunction(f.boxCrossed)&&f.boxCrossed(c)&&(c.y+=f.height,d=!0)}if(d&&(c.x+c.width>b.parent.width||c.y+c.height>b.parent.height))d={x:c.x,y:c.y},b.parent.removeChild(c),this.engine[a](d,b.parent);this.engine.addSequenceFlow({startNode:b,endNode:c});b.state="none";b.selected=!1;b.selectorPainter.visible=!1}},showAt:function(a){if(this.menuDom)this.eventPos=Utils.epos(a),$(this.menuDom).css({left:this.eventPos.x+$(this.engine.container).offset().left+(Utils.isIE7||Utils.isIE6?-30:0),top:this.eventPos.y+
$(this.engine.container).offset().top}).slideDown(),this.isShow=!0},hide:function(){$(this.menuDom).hide();this.isShow=!1}};(function(){window.lastMouseDownTarget=null;$(document).bind("mousedown",function(a){window.lastMouseDownTarget=a.target})})();
...
Engine.prototype.saveAsImage=function(){var a=this.canvas.toDataURL("image/png").replace("image/png","image/octet-stream;Content-Disposition:attachment;filename=canvas.png");console.log(a);window.location.href=a};
...
Engine.prototype._addToCt=function(a,b,c){for(var d=this.stage.children.slice(0),e=!1,f=0;f<d.length;f++){var g=d[f];if(g.baseType=="container"&&g.hitTest(b,c)){a.x=b-g.x;a.y=c-g.y;g.addItem(a);e=!0;break}}return e};
Engine.prototype._addAcitivty=function(a,b,c){var b=this._doAddArgs(b),d=null;try{if(d=eval(a),!d||!Utils.isFunction(d.create))return Q.trace("\u65e0\u6cd5\u521b\u5efa"+a+", \u6216\u8005\u6ca1\u6709create\u51fd\u6570\uff01"),null}catch(e){return Q.trace(e),null}a=d.create(b);d=!1;c?(c.addChild(a),d=!0):d=this._addToCt(a,b.x,b.y);d||this.stage.addChild(a);return a};
